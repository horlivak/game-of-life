...
namespace ECSPrefix202601\React\Dns\Query;

use ECSPrefix202601\React\Dns\Model\Message;
use ECSPrefix202601\React\Dns\Protocol\BinaryDumper;
use ECSPrefix202601\React\Dns\Protocol\Parser;
use ECSPrefix202601\React\EventLoop\Loop;
use ECSPrefix202601\React\EventLoop\LoopInterface;
use ECSPrefix202601\React\Promise\Deferred;
/**
...
final class UdpTransportExecutor implements ExecutorInterface
{
    ...
    public function query(Query $query)
    {
        ...
        if (isset($queryData[$this->maxPacketSize])) {
            return \ECSPrefix202601\React\Promise\reject(new \RuntimeException('DNS query for ' . $query->describe() . ' failed: Query too large for UDP transport', \defined('SOCKET_EMSGSIZE') ? \SOCKET_EMSGSIZE : 90));
        }
        ...
        if ($socket === \false) {
            return \ECSPrefix202601\React\Promise\reject(new \RuntimeException('DNS query for ' . $query->describe() . ' failed: Unable to connect to DNS server ' . $this->nameserver . ' (' . $errstr . ')', $errno));
        }
        ...
        if ($written !== \strlen($queryData)) {
            return \ECSPrefix202601\React\Promise\reject(new \RuntimeException('DNS query for ' . $query->describe() . ' failed: Unable to send query to DNS server ' . $this->nameserver . ' (' . $errstr . ')', $errno));
        }
        $loop = $this->loop;

...
class CyclomaticComplexitySniff implements Sniff
{
     ...
     */
    private const PREDICATE_NODES = [\T_CASE => \true, \T_DEFAULT => \true, \T_CATCH => \true, \T_IF => \true, \T_FOR => \true, \T_FOREACH => \true, \T_WHILE => \true, \T_ELSEIF => \true, \T_INLINE_THEN => \true, \T_COALESCE => \true, \T_COALESCE_EQUAL => \true, \T_MATCH_ARROW => \true, \T_NULLSAFE_OBJECT_OPERATOR => \true];
    /**
    ...
    public function process(File $phpcsFile, int $stackPtr)
    {
        ...
        for ($i = $start + 1; $i < $end; $i++) {
            if (isset(self::PREDICATE_NODES[$tokens[$i]['code']]) === \true) {
                $complexity++;
            }
        }
